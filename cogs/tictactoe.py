...
class Board:
    def __init__(self, player1, player2):
        # Our board just needs to be  3x3 grid. To keep formatting nice, each one isgoing to be a space to start
        self.board = [[" ", " ", " "], [" ", " ", " "], [" ", " ", " "]]
...
class TicTacToe(commands.Cog):
    ...
    @commands.group(aliases=["tic", "tac", "toe", "ttc"], invoke_without_command=True)
    @commands.guild_only()
    async def tictactoe(self, ctx, *, option: str):
        ...
        if not board:
            await ctx.send("There are currently no Tic-Tac-Toe games setup! Type: `;tictactoe start @<player 2>` to start one!")
            return
        ...
        if winner:
            ...
            await utils.update_records("tictactoe", ctx.client.db, winner, loser)
